...
!(function (d) {
  const journalArticles = document.getElementsByClassName(
    "journalpage__article"
  );
  for (let i = 0; i < journalArticles.length; i++) {
    ...
    for (let k = 0; k < paragraphChildrenNodes.length; k++) {
      if (k > 0) {
        paragraphChildrenNodes[
          k
        ].className += `journalpage__article-element--body-${i}`;
        paragraphChildrenNodes[
          k
        ].className += ` journalpage__article-element--hidden`;
      }
    }
    ...
    if (images.length > 1) {
      ...
      carouselFooterContainer.classList.add(
        "journalpage__image-carousel-footer-container"
      );
      carouselFooterContainer.id = `JournalCarouselFooterContainer-${i}`;

      const previousImageButton = document.createElement("button");
      previousImageButton.classList.add(
        "journalpage__carousel-previous-image-button"
      );
      ...
      imageCarousel.classList.add("journalpage__image-carousel--short");
    }
...
document.addEventListener(
  ...
  function (event) {
    ...
    if (event.target.innerHTML === "Read More") {
      //Expand Case
      const journalArticleElementsHidden = document.getElementsByClassName(
        `journalpage__article-element--body-${articleIndex}`
      );
      for (let i = 0; i < journalArticleElementsHidden.length; i++) {
        journalArticleElementsHidden[i].classList.remove(
          `journalpage__article-element--hidden`
        );
      }
      ...
      for (let i = 0; i < journalArticleElementsHidden.length; i++) {
        journalArticleElementsHidden[i].classList.add(
          `journalpage__article-element--hidden`
        );
      }
...
document.addEventListener(
  ...
  function (event) {
    // If the clicked element doesn't have the right selector, bail
    if (
      !event.target.matches(".journalpage__carousel-previous-image-button") &&
    ...
    const carouselFooterCounter = document.getElementById(
      `JournalCarouselImageCounter-${carouselIndex}`
    );
    const carouselFooterIndexAndTotal =
      carouselFooterCounter.textContent.split("/");
    ...
    const activeImage = document.getElementById(
      `JournalArticleImage-${carouselIndex}-${imageIndex}`
    );
    const previousPreviousImage = document.getElementById(
      `JournalArticleImage-${carouselIndex}-${previousPreviousImageIndex}`
    );
    const previousImage = document.getElementById(
      `JournalArticleImage-${carouselIndex}-${previousImageIndex}`
    );
    const nextImage = document.getElementById(
      `JournalArticleImage-${carouselIndex}-${nextImageIndex}`
    );
...
document.addEventListener(
  ...
  function (event) {
    // If the clicked element doesn't have the right selector, bail
    if (
      !event.target.matches(".journalpage__carousel-next-image-button") &&
    ...
    const nextNextImage = document.getElementById(
      `JournalArticleImage-${carouselIndex}-${nextNextImageIndex}`
    );
